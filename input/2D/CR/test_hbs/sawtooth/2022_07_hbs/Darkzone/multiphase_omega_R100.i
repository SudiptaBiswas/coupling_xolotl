...
[Functions]
  [ic_func_etab0]
    ...
    value = 'r:=sqrt((x-200)^2+(y-200)^2);0.5*(1.0-tanh((r-100)*sqrt(mu/2.0/kappa)))'
  []
  [ic_func_etam0]
    type = ParsedFunction
    vars = 'kappa  mu'
    vals = '0.5273 0.004688'
    value = 'r:=sqrt((x-200)^2+(y-200)^2);0.5*(1.0+tanh((r-100)*sqrt(mu/2.0/kappa)))*0.5*(1.0+tanh((y-200)*sqrt(mu/2.0/kappa)))'
  []
  [ic_func_etam1]
    type = ParsedFunction
    vars = 'kappa  mu'
