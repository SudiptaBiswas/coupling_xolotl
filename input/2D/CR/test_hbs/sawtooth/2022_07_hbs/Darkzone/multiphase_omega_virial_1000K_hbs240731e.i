...
[ICs]
  ...
  [IC_wv]
    type = ConstantIC
    variable = wv
    value = 4.484936827853519e-05
  []
  [IC_wg]
    type = ConstantIC
    variable = wg
    value = -0.00356786852796
  []
[]
...
[Functions]
  ...
  [ic_func_etam0]
    ...
    vars = 'kappa  mu'
    vals = '0.5273 0.004688'
    value = 'r:=sqrt((x-200)^2+(y-200)^2);0.5*(1.0+tanh((r-44)*sqrt(mu/2.0/kappa)))*0.5*(1.0+tanh((y-200)*sqrt(mu/2.0/kappa)))'
  []
  [ic_func_etam1]
    type = ParsedFunction
    vars = 'kappa  mu'
