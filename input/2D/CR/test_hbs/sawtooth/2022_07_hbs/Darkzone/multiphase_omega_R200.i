...
[ICs]
  ...
  [IC_wv]
    type = ConstantIC
    variable = wv
    value = 7.698071175206016e-05
  []
  [IC_wg]
    type = ConstantIC
    variable = wg
    value = -0.0039272047254
  []
[]
...
[Functions]
  [ic_func_etab0]
    ...
    value = 'r:=sqrt((x-400)^2+(y-300)^2);0.5*(1.0-tanh((r-200)*sqrt(mu/2.0/kappa)))'
  []
  [ic_func_etam0]
    type = ParsedFunction
    vars = 'kappa  mu'
    vals = '0.5273 0.004688'
    value = 'r:=sqrt((x-400)^2+(y-300)^2);0.5*(1.0+tanh((r-200)*sqrt(mu/2.0/kappa)))*0.5*(1.0+tanh((y-300)*sqrt(mu/2.0/kappa)))'
  []
  [ic_func_etam1]
    type = ParsedFunction
    vars = 'kappa  mu'
